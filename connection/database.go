...
func getConn() *sql.DB {
	if viper.GetString("mysql.host") == "" {
		...
		db, err := sql.Open("sqlite", utils.FileSafe("./db/chatnio.db"))
		if err != nil {
			panic(err)
		}
...
func MigrateRootUser(db *sql.DB) {
	username := configuredRootUsername()
	if username != "root" && IsUserExist(db, "root") && !IsUserExist(db, username) {
		globals.Debug(fmt.Sprintf("[service] migrating root user to %s", username))
		_, err := globals.ExecDb(db, `
			UPDATE auth SET username = ?, password = ?, email = ? WHERE username = 'root'
		`, username, utils.Sha2Encrypt(configuredRootPassword()), configuredRootEmail())
		if err != nil {
			globals.Warn(fmt.Sprintf("[service] failed to migrate root user: %s", err.Error()))
		}
	}
}
