...
func NewConversation(db *sql.DB, id int64) *Conversation {
	DeleteExpiredConversations(db, id)
...
func ExtractConversation(db *sql.DB, user *auth.User, id int64, ref string) *Conversation {
	...
	DeleteExpiredConversations(db, user.GetID(db))
...
func (c *Conversation) GetVideoSeconds() *string {
	return c.VideoSeconds
}

func (c *Conversation) SetVideoSeconds(seconds *string) {
	c.VideoSeconds = seconds
}

func (c *Conversation) GetVideoSize() *string {
	return c.VideoSize
}

func (c *Conversation) SetVideoSize(size *string) {
	c.VideoSize = size
}
