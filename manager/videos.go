...
func VideosRelayAPI(c *gin.Context) {
	...
	if check := checkMarketModelEnabled(form.Model); check != nil {
		sendErrorResponse(c, check, "model_disabled_error")
		return
	}
...
func VideosContentRelayAPI(c *gin.Context) {
	...
	err := globals.QueryRowDb(db, `
		...
		LIMIT 1
	`, userId, id).Scan(&jobModel, &updatedAt)
	...
	if conversation.IsConversationExpiredValue(model, updatedAt) {
		abortWithErrorResponse(c, fmt.Errorf("video job %s has expired after %d days", id, conversation.GetConversationRetentionDays(model)), "invalid_request_error")
		return
	}
