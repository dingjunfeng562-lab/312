...
export function loadPreferenceModels(models: Model[]): Model[] {
  models = models.filter(
    (item) =>
      item.id.length > 0 && item.name.length > 0 && item.enabled !== false,
  );
    ...
    return parsed
      .map((item): Model | null => {
        ...
        return {
          ...
          response_speed: item.response_speed || "standard",
          model_type: item.model_type || "chat",
          function_calling: item.function_calling || false,
          vision_model: item.vision_model || false,
          ocr_model: item.ocr_model || false,
          reverse_model: item.reverse_model || false,
          thinking_model: item.thinking_model || false,
          avatar: item.avatar || "",
          tag: item.tag || [],
          price: item.price,
        } as Model;
