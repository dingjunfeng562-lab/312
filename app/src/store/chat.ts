...
import { getModelType, inferModelType } from "@/api/v1.ts";
...
export type { VideoAspectRatio, VideoDuration, VideoResolution } from "@/conf/video.ts";
...
const chatSlice = createSlice({
  ...
  initialState: {
    ...
    videoAspectRatio: getMemory("videoAspectRatio", defaultVideoSettings.aspectRatio) as VideoAspectRatio,
    videoDuration: getMemory("videoDuration", defaultVideoSettings.duration) as VideoDuration,
    videoResolution: getMemory("videoResolution", defaultVideoSettings.resolution) as VideoResolution,
  } as initialStateType,
  reducers: {
    ...
    setVideoAspectRatio: (state, action) => {
      state.videoAspectRatio = action.payload;
      setMemory("videoAspectRatio", action.payload);
    },
    setVideoDuration: (state, action) => {
      state.videoDuration = action.payload;
      setMemory("videoDuration", action.payload);
    },
    setVideoResolution: (state, action) => {
      state.videoResolution = action.payload;
      setMemory("videoResolution", action.payload);
    },
...
export const selectVideoAspectRatio = (state: RootState): VideoAspectRatio => state.chat.videoAspectRatio;
export const selectVideoDuration = (state: RootState): VideoDuration => state.chat.videoDuration;
export const selectVideoResolution = (state: RootState): VideoResolution => state.chat.videoResolution;
...
export function useMessageActions() {
  ...
  const videoAspectRatio = useSelector(
    (state: RootState) => state.chat.videoAspectRatio,
  );
  const videoDuration = useSelector(
    (state: RootState) => state.chat.videoDuration,
  );
  const videoResolution = useSelector(
    (state: RootState) => state.chat.videoResolution,
  );
  const supportModels = useSelector(selectSupportModels);

  function getVideoPayload(requestModel: string) {
    const currentModel = supportModels.find((item) => item.id === requestModel);
    const modelType = currentModel
      ? getModelType(currentModel)
      : inferModelType(requestModel);

    if (modelType !== "video") return {};

    return getVideoRequestSettings(requestModel, {
  ...
  return {
    send: async (message: string, using_model?: string) => {
      const requestModel = using_model || model;
