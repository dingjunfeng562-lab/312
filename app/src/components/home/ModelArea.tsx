...
function formatModel(
  ...
  badge.push({
    variant: `model-${modelType}`,
    icon: <Icon icon={modelTypeIcons[modelType]} className={`h-3 w-3`} />,
    tooltip: `${t("market.model-type")}: ${t(
      `market.model-type-${modelType}`,
    )}`,
  });
  badge.push({
    variant: `response-${responseSpeed}`,
    icon: <Gauge className={`h-3 w-3`} />,
    tooltip: `${t("market.response-speed")}: ${t(
