...
const responseSpeedClasses: Record<string, string> = {
  fast: "text-yellow-600 bg-yellow-500/10 border-yellow-500/20",
  standard: "text-sky-600 bg-sky-500/10 border-sky-500/20",
  slow: "text-orange-600 bg-orange-500/10 border-orange-500/20",
};

const modelTypeClasses: Record<ModelType, string> = {
  chat: "text-slate-600 bg-slate-500/10 border-slate-500/20",
  image: "text-fuchsia-600 bg-fuchsia-500/10 border-fuchsia-500/20",
  video: "text-violet-600 bg-violet-500/10 border-violet-500/20",
};
...
function ModelItem({
  ...
  const responseSpeed = getModelResponseSpeed(model);
  ...
  return (
    ...
    >
      ...
      <motion.div
        className={`ml-1 mb-1 flex flex-row flex-wrap items-center gap-1.5 text-xs`}
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <span
          className={cn(
            ...
            modelTypeClasses[modelType],
          )}
        >
          <Icon icon={modelTypeIcons[modelType]} className={`w-3 h-3 mr-1`} />
          {t("market.model-type")}: {t(`market.model-type-${modelType}`)}
        </span>
        <span
          className={cn(
            "inline-flex items-center px-1.5 py-0.5 rounded-md border",
            responseSpeedClasses[responseSpeed],
          )}
        >
          <Gauge className={`w-3 h-3 mr-1`} />
          {t("market.response-speed")}:{" "}
          {t(`market.response-speed-${responseSpeed}`)}
        </span>
      </motion.div>
...
function MarketPlace({ search, showPricing, show1mPricing }: MarketPlaceProps) {
  ...
  const models = useMemo(() => {
    ...
    return supportModels.filter((model) => {
      ...
      const responseSpeed = getModelResponseSpeed(model);
      const responseSpeedName = `${t("market.response-speed")} ${t(
        `market.response-speed-${responseSpeed}`,
      )}`.toLowerCase();
      const modelType = getModelType(model);
      const modelTypeName = `${t("market.model-type")} ${t(
        `market.model-type-${modelType}`,
      )}`.toLowerCase();
      ...
      return raw.every(
        (item) =>
          ...
          responseSpeedName.includes(item) ||
