...
function reducer(state: MarketForm, action: any): MarketForm {
  switch (action.type) {
    ...
    case "add-multiple":
      return [
        ...
        ...action.payload.map((model: RawModel) => ({
          ...
          response_speed:
            model.response_speed || inferModelResponseSpeed(model.id || ""),
          model_type: model.model_type || inferModelType(model.id || ""),
      ...
      return [
        {
          ...
          description: "",
          high_context: false,
          response_speed: inferModelResponseSpeed(action.payload.id),
          model_type: inferModelType(action.payload.id),
          default: true,
          tag: [],
          avatar: "",
          seed: generateSeed(),
    ...
    case "batch-new-template":
      return [
        ...action.payload.map((model: { id: string; name: string }) => ({
          ...
          description: "",
          high_context: false,
          response_speed: inferModelResponseSpeed(model.id),
          model_type: inferModelType(model.id),
          default: true,
          tag: [],
          avatar: "",
          seed: generateSeed(),
        ...
        ...state.map((model, idx) => {
          if (idx === action.payload.idx) {
            return {
              ...
              id: action.payload.id,
              response_speed:
                model.response_speed ||
                inferModelResponseSpeed(action.payload.id),
              model_type: model.model_type || inferModelType(action.payload.id),
            };
          }
          return model;
        }),
    ...
    case "update-response-speed":
      return [
        ...state.map((model, idx) => {
          if (idx === action.payload.idx) {
            return { ...model, response_speed: action.payload.response_speed };
          }
          return model;
        }),
      ];
    case "update-model-type":
      return [
        ...state.map((model, idx) => {
          if (idx === action.payload.idx) {
            return { ...model, model_type: action.payload.model_type };
          }
          return model;
        }),
      ];
    ...
    case "update-enabled":
      return [
        ...state.map((model, idx) => {
          if (idx === action.payload.idx) {
            return { ...model, enabled: action.payload.enabled };
          }
          return model;
        }),
      ];
...
function MarketItem({
  ...
  return stackedFilled ? (
    ...
    >
      <div className={`model-wrapper`}>
        ...
        >
          ...
          <div className={`market-row`}>
            <span>
              {t("admin.market.model-enabled")}
              <Tips content={t("admin.market.model-enabled-tip")} />
            </span>
            <Switch
              className={`ml-auto`}
          ...
          <div className={`market-row md:!flex-nowrap`}>
            <span>{t("market.model-type")}</span>
            <Combobox
              value={model.model_type || inferModelType(model.id)}
              onChange={(model_type: string) => {
                dispatch({
                  type: "update-model-type",
                  payload: {
                    idx: index,
                    model_type,
                  },
              ...
              className={`max-w-[320px] ml-auto`}
              list={[...modelTypes]}
              listFormatter={(value: string) => t(`market.model-type-${value}`)}
              placeholder={t("market.model-type")}
              hideSearchBar
            />
          </div>
          <div className={`market-row md:!flex-nowrap`}>
            <span>{t("market.response-speed")}</span>
            <Combobox
              value={model.response_speed || inferModelResponseSpeed(model.id)}
              onChange={(response_speed: string) => {
                dispatch({
                  type: "update-response-speed",
                  payload: {
                    idx: index,
                    response_speed,
                  },
              ...
              className={`max-w-[320px] ml-auto`}
              list={[...modelResponseSpeeds]}
              listFormatter={(value: string) =>
                t(`market.response-speed-${value}`)
              }
              placeholder={t("market.response-speed")}
              hideSearchBar
            />
          </div>
          <div className={`market-row`}>
    ...
    >
      ...
      <Switch
        className={`mr-2`}
        aria-label={t("admin.market.model-enabled")}
        checked={model.enabled !== false}
        onCheckedChange={(enabled) => {
          dispatch({
            type: "update-enabled",
