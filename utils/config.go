...
const (
	staticIndexFile         = "./app/dist/index.html"
	staticIndexCacheFile    = "./app/dist/index.cache.html"
	staticManifestFile      = "./app/dist/site.webmanifest"
	staticManifestCacheFile = "./app/dist/site.cache.webmanifest"
)
...
func ApplySeo(title, icon string) {
	...
	content, err := ReadFile(staticIndexFile)
	if err != nil {
	...
	if err := WriteFile(staticIndexCacheFile, content, true); err != nil {
...
func ApplyPWAManifest(content string) {
	...
	if len(content) == 0 {
		...
		var err error
		content, err = ReadFile(staticManifestFile)
		if err != nil {
	...
	if err := WriteFile(staticManifestCacheFile, content, true); err != nil {
...
func ReadPWAManifest() (content string) {
	...
	if text, err := ReadFile(staticManifestCacheFile); err == nil && len(text) > 0 {
		return text
	}

	if text, err := ReadFile(staticManifestFile); err != nil {
		globals.Warn(fmt.Sprintf("[service] failed to read site.webmanifest: %s", err.Error()))
	} else {
		content = text
	}
...
func serveStaticFileWithFallback(c *gin.Context, primary string, fallback string) {
	if IsFileExist(primary) {
		c.File(primary)
		return
	}
...
func RegisterStaticRoute(engine *gin.Engine) {
	...
	engine.GET("/", func(c *gin.Context) {
		serveStaticFileWithFallback(c, staticIndexCacheFile, staticIndexFile)
	})
	...
	engine.GET("/site.webmanifest", func(c *gin.Context) {
		serveStaticFileWithFallback(c, staticManifestCacheFile, staticManifestFile)
	})
