...
func SetModelTypes(types map[string]string) {
	next := map[string]string{}
	for id, modelType := range types {
		id = strings.TrimSpace(id)
		modelType = strings.TrimSpace(modelType)
		if id == "" || modelType == "" {
			continue
		}
		next[id] = NormalizeModelType(modelType)
	}
	ModelTypes = next
}
...
func InferModelType(model string) string {
	...
	if in(normalized, VideoModels) || strings.Contains(normalized, "video") || strings.Contains(normalized, "sora") {
		return ModelTypeVideo
	}

	if strings.Contains(normalized, "gpt-4-dalle") {
		return ModelTypeChat
	}

	if in(normalized, OpenAIDalleModels) ||
		in(normalized, GoogleImagenModels) ||
		strings.Contains(normalized, "image") ||
		strings.Contains(normalized, "imagen") ||
		strings.Contains(normalized, "dall") ||
...
func GetModelType(model string) string {
	if modelType, ok := ModelTypes[model]; ok {
...
func IsOpenAIDalleModel(model string) bool {
	...
	return GetModelType(model) == ModelTypeImage
}
...
func IsVideoModel(model string) bool {
	return GetModelType(model) == ModelTypeVideo
}
