...
func (c *Channel) FetchModels() ([]string, error) {
	data, err := utils.Get(c.GetAutoModelsEndpoint(), map[string]string{
		"Authorization": fmt.Sprintf("Bearer %s", c.GetRandomSecret()),
	}, c.GetProxy())
	if err != nil {
		return nil, err
	}
	...
	if form == nil {
		return nil, errors.New("cannot parse upstream model list")
	}
